...
import {
  addDays,
  addMonths,
  buildOtherAssignmentNote,
  buildShadowShiftAssignmentNote,
  buildVolunteerShadowShiftNote,
  buildMonthCells,
  ...
  startOfDay,
  stripShadowShiftAssignmentNote,
  toDateInputValue,
  toIsoForDateAndTime,
  toTimeInputValue,
...
function getTemplateCacheSignature(templates: ShiftTemplate[]) {
...
function rankAssignmentForDedupe(assignment: ShiftAssignmentDetail) {
  const statusRank = assignment.status === "active" ? 0 : assignment.status === "pending" ? 1 : 2;
  const roleRank =
...
function dedupeShiftAssignmentsByVolunteer(assignments: ShiftAssignmentDetail[]) {
  const byKey = new Map<string, ShiftAssignmentDetail>();

  assignments.forEach((assignment) => {
    const key = assignment.volunteer?.id ? `volunteer:${assignment.volunteer.id}` : `assignment:${assignment.id}`;
    const existing = byKey.get(key);
    if (!existing || rankAssignmentForDedupe(assignment) < rankAssignmentForDedupe(existing)) {
...
function getCompletedShiftEndMs(shiftInstance: VolunteerHoursShiftInstance) {
  ...
  const template = {
    ...
    start_time:
      shiftInstance.template.start_time ??
      (/evening/i.test(shiftInstance.template.title ?? "") ? "16:00" : "09:00"),
    end_time:
      shiftInstance.template.end_time ??
      (/evening/i.test(shiftInstance.template.title ?? "") ? "19:00" : "11:00"),
    rrule: null,
    capacity: null,
    timezone: null,
    description: null,
...
export default function AuthedApp({ session, profile }: AuthedAppProps) {
  ...
  >({});
  const [shadowShiftNumbersByAssignmentId, setShadowShiftNumbersByAssignmentId] = useState<Record<string, number>>({});
  ...
  const [assignOtherMode, setAssignOtherMode] = useState<"other" | "shadow">("other");
    ...
    const fetchTemplates = async () => {
      ...
      if (error || !data) {
        ...
        const templatesChanged =
          getTemplateCacheSignature(templates) !== getTemplateCacheSignature(nextTemplates);
        setTemplates(nextTemplates);
        if (typeof window !== "undefined") {
          writeStoredJson(
            SHIFT_TEMPLATE_CACHE_KEY,
            nextTemplates,
            templatesChanged ? [WEEK_ASSIGNMENTS_CACHE_PREFIX, SHIFT_INSTANCE_CACHE_PREFIX] : [],
          );
        }
      }
  ...
  const fetchWeekAssignments = useCallback(async () => {
    ...
    const hasHydratedCurrentAssignments = weekAssignmentsHydratedKeyRef.current === assignmentCacheKey;

    if (typeof window !== "undefined" && !hasHydratedCurrentAssignments) {
    ...
    if (instanceIds.length === 0) {
      if (fetchSeq === weekAssignmentsFetchSeqRef.current && !hasHydratedCurrentAssignments) {
        setWeekAssignments({});
      }
      return;
    }
    ...
    const assignmentIdsInView = new Set(allAssignments.map((assignment) => assignment.id));
    const volunteerIdsInView = Array.from(
      new Set(
        allAssignments
          .map((assignment) => {
            const normalizedVolunteer = Array.isArray((assignment as { volunteer?: unknown }).volunteer)
              ? ((((assignment as { volunteer?: unknown }).volunteer ?? null) as unknown as Array<
                  ShiftAssignmentDetail["volunteer"]
                >)?.[0] ?? null)
              : assignment.volunteer;
            return normalizedVolunteer?.id ?? (assignment as ShiftAssignmentDetail & { volunteer_id?: string | null }).volunteer_id ?? null;
          })
          .filter((id): id is string => Boolean(id)),
      ),
    );
    if (volunteerIdsInView.length > 0) {
      const { data: shadowRows } = await supabase
        ...
        .in("volunteer_id", volunteerIdsInView)
        .eq("status", "active")
        .ilike("notes", "Shadow Shift:%");

      const shadowNumbers: Record<string, number> = {};
      const rowsByVolunteer = new Map<
        string,
        Array<{
          id: string;
          ...
          notes: string | null;
          shift_instance:
            | { shift_date: string | null; starts_at: string | null; ends_at: string | null }
            | Array<{ shift_date: string | null; starts_at: string | null; ends_at: string | null }>
            | null;
        }>
      >();
      ((shadowRows ?? []) as Array<{
        ...
        if (!row.volunteer_id || !parseOtherAssignmentNote(row.notes).isShadowShift) return;
        const rows = rowsByVolunteer.get(row.volunteer_id) ?? [];
        rows.push(row);
        rowsByVolunteer.set(row.volunteer_id, rows);
      });
      rowsByVolunteer.forEach((rows) => {
        rows
          .sort((left, right) => {
            const leftShift = Array.isArray(left.shift_instance)
              ? (left.shift_instance[0] ?? null)
              : left.shift_instance;
            const rightShift = Array.isArray(right.shift_instance)
              ? (right.shift_instance[0] ?? null)
              : right.shift_instance;
            const timeDiff = getShadowShiftSortMs(leftShift) - getShadowShiftSortMs(rightShift);
            return timeDiff || left.id.localeCompare(right.id);
          })
          .forEach((row, index) => {
            if (assignmentIdsInView.has(row.id)) {
              shadowNumbers[row.id] = index + 1;
            }
          });
      });
    ...
    const dedupedMap = Object.fromEntries(
      Object.entries(map).map(([instanceId, assignments]) => [
        Number(instanceId),
        dedupeShiftAssignmentsByVolunteer(assignments),
      ]),
    );
    const dedupedCacheMap = Object.fromEntries(
      Object.entries(cacheMap).map(([slotKey, assignments]) => [
        slotKey,
        dedupeShiftAssignmentsByVolunteer(assignments),
      ]),
    );
  ...
  const fetchWeekAppointments = useCallback(async () => {
    ...
    if (instanceShifts.length === 0) {
      if (!weekAppointmentsHydratedKeyRef.current) {
    ...
    const appointmentsCacheKey = instanceShifts.map((shift) => shift.instanceId).sort((left, right) => left - right).join(",");
    const hasHydratedCurrentAppointments = weekAppointmentsHydratedKeyRef.current === appointmentsCacheKey;
    if (!hasHydratedCurrentAppointments) {
    ...
    weekAppointmentsHydratedKeyRef.current = appointmentsCacheKey;
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    const pendingResult = isAdminAccount
      ? await fetchPendingNotifications()
      : { data: [] as ShiftAssignmentDetail[], error: null as string | null };
    ...
    if (error) {
      ...
      setNotificationsMessage(
        pendingItems.length > 0
          ? `Showing pending shift requests. Some other notifications could not load: ${error}`
          : error,
      );
    ...
    if (pendingResult.error) {
      setNotificationsMessage(`Pending shift requests could not load: ${pendingResult.error}`);
    }

    const mergedById = new Map<string, AppNotificationItem>();
    items.forEach((item) => {
    ...
    const mergedItems = Array.from(mergedById.values()).sort(
  ...
  const addWeekAssignmentLocally = useCallback((assignment: ShiftAssignmentDetail) => {
    ...
    setWeekAssignments((previous) => {
      ...
      const nextItems = dedupeShiftAssignmentsByVolunteer([
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      if (Number.isFinite(parsed)) {
        setWeekOffset(Math.min(maxWeekOffset, Math.max(MIN_WEEK_OFFSET, parsed)));
      }
    }
  }, [session.user.id, maxWeekOffset]);
  ...
  const monthJumpOptions = useMemo(() => {
    ...
    for (let i = 0; i < 12; i += 1) {
      ...
      const offset = Math.floor(diffInDays(calendarStartWeek, monthStartWeek) / 7);
      options.push({
        key: getMonthKey(monthDate),
        label: monthJumpFormatter.format(monthDate),
        weekOffset: Math.min(maxWeekOffset, Math.max(MIN_WEEK_OFFSET, offset)),
        monthOffset: i,
      });
    }
    return options;
  ...
  const formatShiftAddedNotificationLabel = useCallback(
    (shift: {
      starts_at?: string | null;
      shift_date?: string | null;
      template?: { title?: string | null } | null;
      ...
      const dateLabel =
        parsedDate && !Number.isNaN(parsedDate.getTime())
          ? `${parsedDate.toLocaleDateString("en-US", { weekday: "long" })} ${parsedDate.toLocaleDateString(
              "en-US",
              { month: "long" },
            )} ${getOrdinalDay(parsedDate.getDate())}`
          : "Upcoming";
      const rawTitle = shift.template?.title ?? shift.title ?? "Shift";
      const shiftTitle = rawTitle.trim() || "Shift";
      return `${shiftTitle} ${dateLabel}`;
    },
    [],
  );
  ...
  const renderInteractiveShiftBlock = (shift: ShiftInstance, keyPrefix = "") => {
    ...
    return (
      <div
        key={`${keyPrefix}${shift.id}`}
        className="shift-block"
        data-shift-instance-id={shift.instanceId}
      >
        <div
                  ...
                  onClick={async () => {
                    ...
                    if (!assignment || !hasVolunteer) {
                      ...
                      if (assignment.status === "pending") {
                        ...
                        setNotesDraft(stripShadowShiftAssignmentNote(assignment.notes) ?? "");
                        setNotesMessage("");
                  ...
                  {assignment?.status === "pending" ? (
                    ...
                    <div className="capacity-slot-content">
                      <span className="capacity-slot-name">
                        {parsedOtherNote.isShadowShift
                          ? `Shadow: ${parsedOtherNote.display}`
  ...
  const handleConfirmTakeShift = async () => {
    ...
    if (previousAssignment && previousAssignment.status !== "dropped") {
      setTakeShiftMessage("You are already on this shift!");
      setTakeShiftLoading(false);
      return;
    }

    const saveResult = previousAssignment?.status === "dropped"
      ? await supabase
          .from("shift_assignments")
          .update({
            status: nextStatus,
      ...
      : await supabase
          ...
          .select("id");
    const { data: savedAssignments, error } = saveResult;
  ...
  const handleAssignmentNotesSave = async () => {
    ...
    setNotesSaving(true);
    setNotesMessage("");
    const isShadowShift = parseOtherAssignmentNote(notesTarget.notes).isShadowShift;
    const volunteerName =
      notesTarget.volunteer?.preferred_name || notesTarget.volunteer?.full_name || "Volunteer";
    const nextNote = isShadowShift
      ? buildShadowShiftAssignmentNote(volunteerName, notesDraft.trim())
  ...
  const handleShadowShiftToggle = async () => {
    if (!notesTarget?.volunteer?.id) return;
    setNotesSaving(true);
    setNotesMessage("");
    ...
    const isShadowShift = parseOtherAssignmentNote(notesTarget.notes).isShadowShift;
    const nextNote = isShadowShift
      ? stripShadowShiftAssignmentNote(notesTarget.notes)
      : buildVolunteerShadowShiftNote(volunteerName, notesDraft);
    const { error } = await supabase
      .from("shift_assignments")
      .update({ notes: nextNote })
      .eq("id", notesTarget.id);
    ...
    const updatedTarget = { ...notesTarget, notes: nextNote };
    setNotesTarget(updatedTarget);
    setNotesDraft(isShadowShift ? (nextNote ?? "") : stripShadowShiftAssignmentNote(nextNote) ?? "");
    updateWeekAssignmentsLocally(notesTarget.id, (assignment) => ({
      ...assignment,
      notes: nextNote,
    }));
    setNotesMessage(isShadowShift ? "Shadow shift removed." : "Marked as shadow shift.");
    setNotesSaving(false);
    await fetchWeekAssignments();
  };
  ...
  const handleDeleteAppointment = useCallback(
    async (appointmentId: string) => {
      ...
      if (appointmentsShiftInstanceId) {
        ...
        const eventError = await recordAppointmentNotificationEvent({
          eventType: "deleted",
          actorId: session.user.id,
          appointmentId,
          shiftInstanceId: appointmentsShiftInstanceId,
          title: deletedAppointment?.title ?? "Appointment",
          body: notificationBody,
          color: deletedAppointment?.color ?? null,
          startsAt: deletedAppointment?.starts_at ?? appointmentsShift?.start.toISOString() ?? null,
          endsAt: deletedAppointment?.ends_at ?? null,
          completedAt: deletedAppointment?.completed_at ?? null,
          completionNote: deletedAppointment?.completion_note ?? null,
        });
        if (eventError) {
          setAppointmentsMessage((previous) =>
            previous
              ? `${previous} Notification center event failed: ${eventError}`
              : `Appointment deleted, but notification center event failed: ${eventError}`,
          );
        }
      }
      setAppointmentDeleteId(null);
  ...
  const handleAssignVolunteer = async (volunteerId: string) => {
    ...
    const notes = isShadowAssignment
      ? buildVolunteerShadowShiftNote(volunteerName, existingLocalAssignment?.notes ?? null)
      : undefined;
    ...
    const savedAssignmentNotes = isShadowAssignment
    ...
    if (notificationErrors.length > 0) {
      setAssignMessage(`${assignmentLabel} added, but ${notificationErrors.join(" | ")}`);
      setAssignLoading(false);
  ...
  const handleAssignOther = useCallback(async () => {
    ...
    const notes = buildOtherAssignmentNote(label, details);
    ...
    const entryLabel = "Other";
    const adminPushError = await sendAdminPush({
      title: "Shift updated",
      body: `${entryLabel}: ${label} was added to ${shiftTitle}, ${shiftDate} (${shiftTime}).`,
    });
    const leadNotifyError = await notifyLeadsOnShiftInstance({
      shiftInstanceId: assignShiftInstanceId,
      excludeVolunteerIds: [session.user.id],
      title: "Shift updated",
      body: `${entryLabel}: ${label} was added to ${shiftTitle}.`,
      notificationType: "shift_added",
    });
    ...
    setAssignMessage(
      notificationErrors.length > 0
        ? `Added ${entryLabel}, but ${notificationErrors.join(" | ")}`
        : `Added ${entryLabel} to this shift.`,
    );
    setShiftInstancesRefreshToken((value) => value + 1);
    refreshShiftViewsInBackground();
  }, [
  ...
  const handleNotificationDecision = async (
    ...
    const approvedShiftLabel = formatShiftAddedNotificationLabel(approvedRequest?.shift_instance);
  ...
  const handleRemoveVolunteer = async () => {
    ...
    const removeTargetLabel = parseOtherAssignmentNote(removeTarget.notes).name;
    const volunteerName =
  ...
  const getClosestUpcomingShiftTarget = () => {
    ...
    const visibleUpcomingShift = instanceShifts
      .filter((shift) => shift.end.getTime() > now.getTime())
      .sort((left, right) => {
        const startDiff = left.start.getTime() - right.start.getTime();
        if (startDiff !== 0) return startDiff;
        return left.title.localeCompare(right.title);
    ...
    if (visibleUpcomingShift) {
      return {
        ...
        shiftInstanceId: visibleUpcomingShift.instanceId,
      };
    }
    ...
    for (let dayOffset = 0; dayOffset <= 366; dayOffset += 1) {
      ...
      activeTemplates.forEach((template) => {
        ...
        const endsAtIso = toIsoForDateAndTime(day, resolveTemplateEndTime(template));
  ...
  const scrollDayCardToShiftSection = (
    ...
    const targetByInstanceId =
      shiftInstanceId != null
        ? shiftBlocks.find((block) => block.dataset.shiftInstanceId === String(shiftInstanceId)) ?? null
        : null;
  ...
  const scrollToDateKey = (
    dateKey: string,
    attempt = 0,
    options?: { focusShiftSection?: "morning" | "evening" | null; focusShiftInstanceId?: number | null },
  ) => {
    ...
    if (options?.focusShiftSection) {
      window.setTimeout(() => {
        scrollDayCardToShiftSection(target, options.focusShiftSection, options.focusShiftInstanceId);
      }, 140);
    }
  };
  ...
  useEffect(() => {
    ...
    const shiftSectionPreference =
      target.dateKey === targetKey ? target.preference : getTodayShiftScrollPreference();
    const focusShiftInstanceId = target.dateKey === targetKey ? target.shiftInstanceId : null;
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        scrollToDateKey(targetKey, 0, {
          focusShiftSection: shiftSectionPreference,
          focusShiftInstanceId,
        });
        window.setTimeout(
          () =>
            scrollToDateKey(targetKey, 0, {
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        scrollToDateKey(target.dateKey, 0, {
          focusShiftSection: target.preference,
          focusShiftInstanceId: target.shiftInstanceId,
        });
        window.setTimeout(
          () =>
            scrollToDateKey(target.dateKey, 0, {
  ...
  const handleTodayClick = () => {
    ...
    const target = getClosestUpcomingShiftTarget();
    const shiftSectionPreference =
      target.dateKey === nowKey ? target.preference : getTodayShiftScrollPreference();
    const focusShiftInstanceId = target.dateKey === nowKey ? target.shiftInstanceId : null;

    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        scrollToDateKey(nowKey, 0, {
          focusShiftSection: shiftSectionPreference,
          focusShiftInstanceId,
        });
        window.setTimeout(
  ...
  const handlePrevRange = () => {
    ...
    setWeekOffset((value) => Math.max(MIN_WEEK_OFFSET, value - 1));
  };
  ...
  const isShadowFollowUpNotification = (
            ...
            <div className="calendar-title-with-month">
              ...
              <div className="month-title-nav">
                <button
                  ...
                  disabled={calendarRangeMode === "week" && weekOffset <= MIN_WEEK_OFFSET}
                  aria-label="Previous"
                  title="Previous"
                >
                  ←
              ...
              {notifications.length > 0 ? (
                <motion.div className="notifications-list" layout={prefersReducedMotion ? false : "position"}>
                  ...
                  {notifications.map((request, index) => {
                    ...
                    if (isShadowFollowUpNotification(request)) {
                      const volunteerName =
                        request.shadow_name ||
                      ...
                      return renderNotificationCard(
                        ...
                        <>
                          ...
                          <p className="notification-name">
                            Second shadow shift done for{" "}
                            <span className={volunteerNameClass}>{volunteerName}</span>
                          </p>
                          <p className="notification-reason">
                            Reach out to ask what weekly shift they want.
                          </p>
                        </>,
                        isMobile ? null : (
                          <>
                            <button
                              className="account-button notification-icon-action"
                              type="button"
                              onClick={(event) => {
                                event.stopPropagation();
                                setVolunteerSearchInput(volunteerName);
                                setSelectedVolunteer(null);
                                setShowVolunteers(true);
                              }}
                              aria-label="Find volunteer"
          ...
          <div className="modal-panel account-panel">
            <div className="modal-header">
              <div>
                ...
                <h3 className="modal-title">
                  {assignOtherMode === "shadow" ? "Add shadow shift" : "Add volunteer to shift"}
                </h3>
              </div>
              <button
                className="modal-close"
            ...
            <div className="modal-body">
              ...
              <div className="modal-actions assign-volunteer-actions">
                <button
                  ...
                  onClick={() => {
                    setAssignOtherMode("other");
                    setShowAssignOtherForm((current) => !(current && assignOtherMode === "other"));
                  }}
                >
                  {showAssignOtherForm && assignOtherMode === "other" ? "Cancel Add Other" : "Add Other"}
                </button>
                <button
                  className={`account-button ${assignOtherMode === "shadow" ? "active" : ""}`}
                  type="button"
                  onClick={() => {
                    setShowAssignOtherForm(false);
                    setAssignOtherMode((current) => (current === "shadow" ? "other" : "shadow"));
                  }}
                >
                  {assignOtherMode === "shadow" ? "Cancel Shadow Shift" : "Add Shadow Shift"}
                </button>
              </div>
              {showAssignOtherForm ? (
                      ...
                      >
                        <div className="volunteer-main">
                          ...
                          <p className="volunteer-meta">
                            {assignOtherMode === "shadow" ? `Shadow shift · ${roleLabel}` : roleLabel}
                          </p>
                        </div>
            ...
            <div className="modal-body">
              <p className="modal-text">
                ...
                >
                  {removeTarget?.volunteer?.preferred_name ||
                    removeTarget?.volunteer?.full_name ||
                    parseOtherAssignmentNote(removeTarget?.notes).name ||
                    "this entry"}
                </span>{" "}
                from this shift?
              </p>
      ...
      {showAssignmentNotes ? (
        <div className="modal-backdrop" role="dialog" aria-modal="true" onClick={handleModalBackdropClick}>
          <div className="modal-panel take-shift-panel volunteer-note-panel">
            ...
            <div className="modal-body">
              {notesTarget?.volunteer?.id ? (
                <button
                  className="account-button"
                  type="button"
                  onClick={() => void handleShadowShiftToggle()}
                  disabled={notesSaving}
                >
                  {parseOtherAssignmentNote(notesTarget.notes).isShadowShift
                    ? "Remove Shadow Shift"
                    : "Make Shadow Shift"}
                </button>
              ) : null}
